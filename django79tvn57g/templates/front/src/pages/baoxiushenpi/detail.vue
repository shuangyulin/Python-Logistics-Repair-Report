	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'·'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/baoxiushenpi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">接收状态</div>
						<div class="text "  >{{detail.jieshouzhuangtai}}</div>
					</div>
					<div class="item">
						<div class="lable">审核时间</div>
						<div class="text "  >{{detail.shenheshijian}}</div>
					</div>
					<div class="item">
						<div class="lable">审核结果</div>
						<div class="text "  >{{detail.shenhejieguo}}</div>
					</div>
					<div class="item">
						<div class="lable">审核意见</div>
						<div class="text "  >{{detail.shenheyijian}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.shenpixingming}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('baoxiushenpi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('baoxiushenpi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('baoxiushenpi','接收')" 
							@click="onAcross('jieshouweixiu','','','jieshouzhuangtai','您的预约已接收','已接收,未接收'.split(',')[0])" type="warning">
							接收
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`baoxiushenpi`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/baoxiushenpi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/baoxiushenpiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此报修审批？') .then(_ => {
					this.$http.post('baoxiushenpi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
