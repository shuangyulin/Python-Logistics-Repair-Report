	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('baoxiushenpi','新增')" type="primary" @click="add('/index/baoxiushenpiAdd')">
					...
					<div v-for="(item, index) in dataList" :key="index" class="list-item animation-box" @click.stop="toDetail(item)">
						...
						<div class="item-info">
							...
							<div class="publisher_item">
								...
								<span class="text">{{item.shenpizhanghao}}</span>
							</div>
							<div class="more_btn">
							  查看详情
							</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`baoxiushenpi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/baoxiushenpiDetail', query: params});
			},
