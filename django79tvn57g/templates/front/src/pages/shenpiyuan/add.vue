	...
	export default {
		data() {
			return {
				...
				rules: {
					shenpizhanghao: [
						{ required: true, message: '审批账号不能为空', trigger: 'blur' },
					],
					mima: [
						{ required: true, message: '密码不能为空', trigger: 'blur' },
					],
					shenpixingming: [
						{ required: true, message: '审批姓名不能为空', trigger: 'blur' },
					],
					xingbie: [
					],
					lianxishouji: [
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`shenpiyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shenpiyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
